...
### Интеграционная проверка TarnVPN на `v1.14.0-lx.24`

Перенос на `v1.14.0-lx.24` вернул апстримовый `AndroidAPI: 24` в основной AAR.
Это ломает сборку TarnVPN `other` с `minSdk 23` на этапе manifest merge. Контракт
восстановлен: основная AAR собирается для API 23, legacy — для API 21. Это безопасно
для текущего набора функций: legacy AAR уже собирает те же XHTTP, AWG, OpenVPN и OpenConnect
теги для API 21. `lx-ci` теперь проверяет оба `minSdkVersion` прямо в собранных AAR.
