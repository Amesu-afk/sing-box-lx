...
На ветках с pooled-XHTTP тот же инвариант обязан соблюдать
`handshakeRoundTrip`: запрос получает внутренний контекст без наследуемой
отмены, а `context.AfterFunc` переносит в него отмену родителя только до
заголовков ответа. После успешного handoff watcher снимается, и внутренний
контекст живёт до `response.Body.Close()`. Прямой `context.WithCancel(req.Context())`
на весь срок body запрещён — обычная отмена завершившегося dial-контекста тогда
мгновенно рвёт уже поднятый `stream-one`.
