...
- **P10. REALITY подключается к актуальному Xray.** Клиент объявляет допустимую
  версию и отправляет обязательный для Xray v26.9.8+ гибридный key share
  `X25519MLKEM768` перед запасным `X25519`, в том числе с fingerprint Firefox.
  Свидетель: red/green тесты ClientHello для Firefox и Chrome; полевой прогон
  TestMiami на Xray v26.9.9. Мутация: возврат фильтра hybrid-группы или пустой
  key share, добавленный после генерации ключей.
...
| [053](../../TASKS/053-REALITY_MIN_CLIENT_VER/SPEC.md) | REALITY не проходит актуальный Xray: v26.7.11+ отсекает старую объявленную версию, а v26.9.8+ требует `X25519MLKEM768`, который sing-box удаляет; с Firefox симптом — `reality verification failed` | `common/tls/reality_client.go`, `common/tls/reality_client_test.go` (`// lx: SPEC 053`) — версия `26.3.27` и hybrid key share перед fallback `X25519` | Апстрим поднимет версию и сохранит/добавит обязательный hybrid key share для всех fingerprint; части фикса снимаются независимо | держим — alpha.52, юниты, lint и field TestMiami/Xray v26.9.9 зелёные |
...
**053 — совместимость REALITY с актуальным Xray.** Сначала в чужом коде
протухла константа. REALITY возит версию клиента в первых трёх
...
В Xray v26.9.8 появился второй барьер: сервер требует гибридный key share
`X25519MLKEM768` перед запасным `X25519`. Апстримный REALITY-клиент после
построения ClientHello намеренно удалял hybrid, а Firefox fingerprint не
предлагал его изначально. Простого добавления ID после `BuildHandshakeState`
недостаточно — uTLS тогда уже не создаёт закрытый ML-KEM ключ. Поэтому spec
исправляется до `ApplyPreset`: uTLS штатно генерирует обе пары ключей, GREASE
остаётся первым, hybrid идёт перед X25519, а Chrome не получает дубль.

Условие снятия здесь пассивное вдвойне. Апстрим однажды поднимет константу и
перестанет вырезать обязательный hybrid сам —
