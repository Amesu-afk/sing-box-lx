...
| [083](../../TASKS/083-REALITY_MLKEM_KEYSHARE/SPEC.md) | REALITY-узлы на сервере Xray ≥ v26.9.8 **молча** уходят на камуфляжный сайт (`reality verification failed`): сервер (`XTLS/REALITY@8cdf7bf`) требует key_share `X25519MLKEM768` перед X25519, а апстрим сам вырезает гибрид из ClientHello (костыль под utls 1.7.2) | `common/tls/reality_client.go` (`// lx: SPEC 083`) — фильтр снят, `AuthKey` по `Ecdhe`, при nil — `MlkemEcdhe` | Апстрим уберёт фильтр сам (issue #4520); проверять на мерже, что фильтр не вернулся и `HelloChrome_Auto` несёт гибрид перед X25519 | держим — DEVICE-VERIFIED на стенде (v26.9.9 ходит, v26.7.11/v26.7.28 без регрессии); не-chrome отпечатки гибрида в `metacubex/utls` не несут — `firefox` закрыт форком [086](../../TASKS/086-UTLS_FORK_FIREFOX148/SPEC.md), `safari` — [087](../../TASKS/087-UTLS_SAFARI_26_3/SPEC.md); `edge`/`ios`/`android`/`360`/`qq` — по решению владельца остаются, подмена в приложениях |
