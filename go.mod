...
// for CI/release once validated against a live AWG server. See SPECS/TASKS/003.
replace github.com/sagernet/wireguard-go => ./submodules/wireguard-go

// lx:end awg

// lx:begin singtun-selfheal (SPECS/TASKS/040)
// Fork of sing-tun based on upstream d67734281390 (the go.mod revision) plus the
// three post-base fixes cherry-picked from sagernet/main (2ce8e08, ac90f98,
// a95f1ae), with a single-file patch: system-stack acceptLoop self-heal — on an
// unexpected Accept error the listener is logged (errno names the killer) and
// recreated instead of the loop silently dying forever (every new TCP → instant
// RST until a VPN restart; LxBox §047). Local-path replace, same scheme as
// wireguard-go.
...
// Snapshot fork of sagernet/gvisor at the pinned v0.0.0-20260727.0-sing-box-mod.1
...
// upstream's pin) plus two commits cherry-picked from refraction-networking/utls:
// fc716b2 (the HelloFirefox_148 preset + reuse of one classical X25519 key between the
// hybrid and the classical key_share entries) and ddebe39 (the same reuse re-done via
// marker bytes in KeyShare.Data instead of an unexported field). REALITY servers on
...
// `fp=firefox` (HelloFirefox_120 there) is silently rejected. The primary source can't
