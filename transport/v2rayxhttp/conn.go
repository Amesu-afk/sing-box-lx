...
func (c *Client) dialStreamOne(ctx context.Context, sessionID string) (net.Conn, error) {
	...
	lease := c.acquireSlot()
	conn := newStreamConn(pipeWriter, c.serverAddr, lease)
	go func() {
...
func (c *Client) dialStreamUp(ctx context.Context, sessionID string) (net.Conn, error) {
	...
	conn := newSplitConn(downResp.Body, pipeWriter, c.serverAddr, lease)
	go func() {
...
func newStreamConn(writer *io.PipeWriter, serverAddr M.Socksaddr, lease *slotLease) *streamConn {
	return &streamConn{
		writer:     writer,
...
func newSplitConn(reader io.ReadCloser, writer *io.PipeWriter, serverAddr M.Socksaddr, lease *slotLease) *splitConn {
	return &splitConn{
		reader:     reader,
		writer:     writer,
		serverAddr: serverAddr,
