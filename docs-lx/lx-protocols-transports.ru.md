...
совместимости: формы `service_name`.
...
## 4.1 `service_name`: формы Xray

У `service_name` **две** формы, ровно как у Xray. Какая из них — решает один
символ: ведущий `/`.

- **Без ведущего `/`** — всё значение это один сегмент сервиса: он экранируется
  целиком (`/` внутри становится `%2F`), имя стрима всегда `Tun`. Это прежняя
  форма, она не изменилась байт в байт.
- **С ведущим `/`** — значение это **custom path** в конвенции Xray: каждый
  сегмент экранируется **по отдельности** (то есть `/` остаётся настоящим
  разделителем пути), **последний** сегмент — имя стрима, а хвост `|…` у имени
  стрима (имя multi-стрима Xray) отбрасывается.

| `service_name` | Путь на проводе | Сервис / стрим | Примечание |
|---|---|---|---|
| `TunService` | `/TunService/Tun` | `TunService` / `Tun` | прежняя форма |
| `a/b` | `/a%2Fb/Tun` | `a%2Fb` / `Tun` | прежняя форма, = старая форма Xray |
| `/a/b/Tun` | `/a/b/Tun` | `a/b` / `Tun` | custom path, = Xray |
| `/a/b/Stream` | `/a/b/Stream` | `a/b` / `Stream` | имя стрима берётся из конфига |
| `/a b/Tun` | `/a%20b/Tun` | `a%20b` / `Tun` | посегментный `PathEscape` |
| `/a/b/Tun\|TunMulti` | `/a/b/Tun` | `a/b` / `Tun` | хвост multi отброшен, как у Xray |

Новой опции нет, валидации нет: форму задаёт содержимое `service_name` — так же,
как у Xray.

**Пометки.**

1. Клиент под тегом **`with_grpc`** для прежней формы путь **не экранирует** (шлёт
   `"/" + name + "/Tun"` как есть, поэтому `a/b` уходит как `/a/b/Tun`). Это
   апстримное поведение, мы его не меняли — разница видна только для имени прежней
   формы, внутри которого есть `/`.
2. Наш **lite-сервер** сравнивает **декодированный** путь, поэтому он мягче Xray:
   сервер с `service_name: "a/b"` принимает и `/a%2Fb/Tun`, и `/a/b/Tun`. Xray
   сравнивает строго — выбирай одну и ту же форму с обеих сторон.
3. **Симптом несовпадения форм** с сервером Xray:
   `v2ray-grpc: unexpected status: 404 Not Found` — dial до сервера дошёл и был
   отвергнут по пути.

**📖 Статус.** Реализовано, покрыто юнитами и тестами «клиент↔сервер»; **против
живого Xray пока не проверено** — прогон запланирован до релиза.
