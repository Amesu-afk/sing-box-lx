...
const (
	...
	// configPreflightLimit bounds the /config.js body drained by
	// preflightConfig so the connection can be reused for the real fetch.
	configPreflightLimit = 128 * 1024
	// configPreflightTimeout bounds the whole probe, retries included.
	configPreflightTimeout = 20 * time.Second
	...
	// Without this, the peer's own recovery can drive both sides into an
	// infinite reconnect ping-pong loop.
...
type Session struct {
	...
	insecure   bool
	bridgeMode string
	// httpClient is handed to the j library so that config discovery, the
	// XMPP WebSocket and colibri-ws all dial through protected sockets.
	...
	pcCancel context.CancelFunc // cancels pcCtx when the live PC is replaced
	...
	reconnectMu          sync.Mutex // guards reconnectWindowStart, reconnectCount and lastReconnectAt
...
func New(_ context.Context, cfg engine.Config) (engine.Session, error) {
	...
	s := &Session{
		...
		name:          name,
		insecure:      insecure,
		httpClient:    newProtectedHTTPClient(insecure),
...
// browserUserAgent is the User-Agent j already sends on the XMPP WebSocket
// handshake. The /config.js fetch has to match it — see browserHeaderTransport.
const browserUserAgent = "Mozilla/5.0 (X11; Linux x86_64; rv:150.0) Gecko/20100101 Firefox/150.0"

// browserHeaderTransport fills in browser-like request headers when the caller
// left them unset.
//
// j hardcodes such headers on the WebSocket handshake but sends the /config.js
// fetch bare, so the two requests look like different clients to whatever sits
// in front of the deployment. This only aligns them; it is hardening, not a
// fix for any one host.
...
func (t *browserHeaderTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if err != nil {
		return resp, fmt.Errorf("browser round trip: %w", err)
	}
	return resp, nil
}

// newProtectedHTTPClient builds the HTTP client the j library uses for
// /config.js discovery, the XMPP WebSocket and colibri-ws.
//
// Without it j falls back to http.DefaultClient, whose sockets are not passed
// to VpnService.protect and therefore loop back into our own tunnel: the
// /config.js fetch fails, j silently falls back to xmppDomain = web host, and
// Prosody answers host-unknown ("This server does not serve <host>") on
// deployments whose XMPP virtualhost differs from the web host — every
// docker-jitsi-meet install, where it is "meet.jitsi".
//
// The cookie jar carries the session cookie such a WAF hands out on the first
// request over to the WebSocket handshake, the way a browser would.
func newProtectedHTTPClient(insecure bool) *http.Client {
	client := protect.NewHTTPClient()
	if insecure {
		client = protect.NewInsecureHTTPClient()
	}
	client.Transport = &browserHeaderTransport{base: client.Transport}
	if jar, err := cookiejar.New(nil); err == nil {
		client.Jar = jar
	} else {
		logger.Warnf("jitsi: cookie jar unavailable: %v", err)
	}
	return client
}
...
func (s *Session) Connect(ctx context.Context) error {
	...
	if err := s.preflightConfig(ctx); err != nil {
		return err
	}
...
// preflightConfig fetches /config.js before joining so that an unreachable
// host is reported as such.
//
// j discovers the XMPP virtualhost from /config.js and silently falls back to
// the web host when the fetch fails; on docker-jitsi-meet that fallback is
// always wrong ("meet.jitsi" is the real virtualhost) and Prosody answers
// host-unknown, which reads like a server misconfiguration rather than a
// failed request. A non-200 answer is only logged: deployments mounted under
// a path prefix legitimately serve no /config.js at the root, and j copes
// with that on its own.
func (s *Session) preflightConfig(ctx context.Context) error {
	// Bound the probe so that retries against a hanging host cannot eat the
	// caller's whole connect budget; j still has its own fetch to make.
	ctx, cancel := context.WithTimeout(ctx, configPreflightTimeout)
	defer cancel()

	url := "https://" + s.host + "/config.js"
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, url, nil)
	if err != nil {
		return fmt.Errorf("jitsi config request: %w", err)
	}
	resp, err := s.httpClient.Do(req)
	if err != nil {
		return fmt.Errorf("jitsi config fetch %s: %w", url, err)
	}
	defer func() { _ = resp.Body.Close() }()
	_, _ = io.Copy(io.Discard, io.LimitReader(resp.Body, configPreflightLimit))
	if resp.StatusCode != http.StatusOK {
		logger.Warnf("jitsi: %s returned status %d; relying on j defaults", url, resp.StatusCode)
	}
	return nil
}
...
func (s *Session) negotiatePC(ctx context.Context, jSess *j.Session, sctpBridge bool) error {
	...
	// teardownPC cancels pcCancel so goroutines bound to pcCtx exit before
	// a fresh PC takes its place.
	if s.pcCancel != nil {
		s.pcCancel()
	}
...
// transport keeps observing application traffic while the engine is idle.
...
func (s *Session) acceptEpochFrame(payload []byte) ([]byte, bool) {
	...
	// a signal that this side must reconnect. If the peer recovered and emits
	// a fresh epoch, accept the new epoch instead of starting a reconnect ping-
	// pong cycle.
...
func (s *Session) handleReconnectAttempt(ctx context.Context) bool {
	// Track consecutive failures, not total reconnect events over the session.
	// A successful reconnect resets the counter so long-lived sessions can
	// recover from legitimate peer churn and JVB restarts.
	for {
		s.reconnectMu.Lock()
...
func (s *Session) reconnect(ctx context.Context) error {
	...
	// Re-establish the XMPP/MUC session from scratch instead of using the
	// lightweight Rejoin path. Fresh JoinMUC re-runs focus allocation, which is
	// more reliable after the room was left idle and Jicofo expired state.
...
func (s *Session) reconnectFull(ctx context.Context) error {
	...
	bcancel()
	if err != nil {
		// No peer may be present yet. Park the fresh MUC session and restart the
		// async waiter instead of counting this as a reconnect failure.
		s.jSess.Store(jSess)
		s.wg.Add(1)
		go s.waitForJingle()
		return errNoPeer
