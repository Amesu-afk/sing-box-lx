...
func (c *Client) bringUpLink(
	...
	if err := waitForSendable(ctx, ln); err != nil {
		return err
	}
...
// sendableWaitTimeout bounds the wait for a writable carrier. It matches the
// window smux itself spends on the first frame, so the timing is unchanged —
// only the diagnosis improves.
const sendableWaitTimeout = 30 * time.Second

// waitForSendable blocks until the carrier can actually put bytes on the wire.
//
// A peer being present in the room does not mean there is a channel to it yet:
// on Jitsi the bridge only opens once Jicofo sends session-initiate and the
// colibri-ws or SCTP path comes up. Until then muxconn.Write spins, and the
// first smux frame fails with a bare "open control stream: timeout" that reads
// like the server never answered — when in fact nothing was ever sent.
func waitForSendable(ctx context.Context, ln transport.Transport) error {
	const pollInterval = 50 * time.Millisecond

	deadline := time.Now().Add(sendableWaitTimeout)
	for {
		if ln.CanSend() {
			return nil
		}
		if time.Now().After(deadline) {
			return fmt.Errorf(
				"carrier bridge not ready after %s: peer joined but no data channel to it was opened",
				sendableWaitTimeout,
			)
		}
		select {
		case <-ctx.Done():
			return fmt.Errorf("wait for carrier bridge: %w", ctx.Err())
