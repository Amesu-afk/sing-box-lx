...
func NewHTTPClient() *http.Client {
	return newHTTPClient(NewHTTPTransport())
}

// NewInsecureHTTPClient returns NewHTTPClient with certificate verification
// disabled, for profiles that explicitly opt into an untrusted Jitsi host.
func NewInsecureHTTPClient() *http.Client {
	transport := NewHTTPTransport()
	transport.TLSClientConfig.InsecureSkipVerify = true //nolint:gosec // opt-in per profile
...
// WinSock reports a reset under its own error numbers, and their syscall
// constants exist only in the windows build. Desktop builds and tests would
// otherwise classify a reset differently from the Android target, where it
// arrives as ECONNRESET. The numbers never collide with a real errno on the
// platforms this code ships to.
const (
	wsaeConnAborted = syscall.Errno(10053)
	wsaeConnReset   = syscall.Errno(10054)
)

// isRetriableError reports whether err is a transient network failure.
//
// Every branch here must fall through rather than return early: a peer reset
// arrives wrapped in a *net.OpError, so classifying OpError by timeout alone
// used to answer false and a single reset killed the whole connection attempt
// — the failure mode seen against flaky Jitsi hosts, which reset conversations
// at random and serve the next attempt fine.
func isRetriableError(err error) bool {
	...
	if errors.As(err, &opErr) && (opErr.Timeout() || strings.Contains(opErr.Error(), "connection refused")) {
		return true
	}
	s := err.Error()
